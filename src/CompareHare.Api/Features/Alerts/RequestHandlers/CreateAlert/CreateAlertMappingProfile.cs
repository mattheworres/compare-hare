    ...
    public class CreateAlertMappingProfile : Profile
    {
        public CreateAlertMappingProfile() {
            ...
            CreateMap<CreateAlertModel, Alert>()
                ...
                .ForMember(d => d.HasRenewable, mce => mce.MapFrom(s => s.FilterRenewable ? (bool?)s.HasRenewable : null))
                ...
                .ForMember(d => d.HasNetMetering, mce => mce.MapFrom(s => s.FilterNetMetering ? (bool?)s.HasNetMetering : null))
                .ForMember(d => d.MinimumMonthLength, mce => mce.MapFrom(s => s.HasMinimumMonthLength && s.MinimumMonthLength != 0 ? (int?)s.MinimumMonthLength : null))
                .ForMember(d => d.MaximumMonthLength, mce => mce.MapFrom(s => s.HasMaximumMonthLength ? (int?)s.MaximumMonthLength : null))
