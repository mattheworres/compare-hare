  ...
  public class AlertAssessor : IAlertAssessor
    {
        ...
        public async Task<AlertAssessorReturnModel> AssessMatches(int alertId)
        {
            ...
            if (!string.IsNullOrEmpty(alert.StateUtilityIndexHash) && sui != null && alert.StateUtilityIndexHash == sui.LastUpdatedHash) {
            ...
            // TODO: assess if we really need to check for null on sui here...
            ...
            foreach(var id in matchingHistoryIds) {
                var item = await _dbContext.UtilityPriceHistories.FindAsync(id);
        ...
        private async Task<IEnumerable<int>> GetMatches(Alert alert, StateUtilityIndex sui) {
            if (sui.UtilityState == UtilityStates.Pennsylvania && sui.UtilityType == UtilityTypes.Power) {
                return await _sqlExecutor.Execute(new GetAlertMatchesForPaPower(alert, sui.Id), null);
            }
        ...
        private async Task RemoveExistingHistories(AlertMatch match) {
            await _sqlExecutor.Execute(new RemoveAllPriceLinkers(match.Id), null);
        }
    }
}
